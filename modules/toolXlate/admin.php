 ...
 * @version    4.x Last Update: 2020-12-02
 * @filesource /bizuno-locale/modules/toolXlate/admin.php
 */
...
function preSubmit() {
    ...
    public function save()
    {
        ...
        if (sizeof($output) > 0) {
            ...
            dbWrite(BIZUNO_DB_PREFIX.$this->moduleID, ['date_create'=>date('Y-m-d')], 'update', "iso='$iso'");
        }
    ...
    public function export()
    {
        ...
        foreach ($langList as $module) {
            ...
            if (isset($output[$module['id']])) {
                $data  = '<'.'?'."php\n/* Translated file created by $this->moduleID on ".date('Y-m-d')." for iso code: $iso */\n".'$lang = ['."\n";
        ...
        $locale = parseXMLstring(file_get_contents(BIZUNO_LIB."locale/en_US/locale.xml"));
    ...
    {
        $langCore= [];
        include(BIZUNO_LIB."locale/en_US/language.php");
        ...
        $lang      = [];
        include(BIZUNO_LIB."locale/en_US/langLocale.php");
        ...
        foreach ($modules as $mID => $path) {
            ...
            $basePath = in_array($mID, $this->core) ? BIZUNO_LIB."locale/en_US/module/$mID/" : BIZUNO_EXT."$mID/locale/en_US/";
    ...
    private function getLang($mID, $path)
    {
        ...
        $fullPath= in_array($module[0], $this->core) ? BIZUNO_LIB."locale/en_US/module/$path" : BIZUNO_EXT."$mID/locale/en_US/".str_replace("{$module[0]}/", '', $path);
    ...
    private function getXlate($mID, $path='', $iso='')
    {
        ...
        if (in_array($mID, ['common'])) { //it's a core file
            $langCore = [];
            if (!file_exists(BIZUNO_LOCALE."$iso/language.php")) { return []; }
            include(BIZUNO_LOCALE."$iso/language.php");
            return $langCore;
        }
        $lang = [];
        $module  = strpos($mID, ':') ? explode(':', $mID, 2) : [$mID];
        $basePath = in_array($module[0], $this->core) ? BIZUNO_LOCALE."$iso/module/$path" : BIZUNO_LOCALE."$iso/ext/$path";
...
//$rows = array_slice($rows, 0, 85);
    ...
    public function getGoogleNext(&$layout=[])
    {
        ...
        if (isset($resp['data']['translations'])) { foreach ($resp['data']['translations'] as $idx => $xlate) {
            ...
            $rows[$idx]['iso'] = $iso;
            $rows[$idx]['date_create'] = date('Y-m-d');
...
bizWindowClose('winNewXlate');"]];
   ...
   private function getModLang($modID='toolXlate')
   {
       ...
       $output = $lang = [];
       msgDebug("\nChecking lang from file: ".BIZUNO_LOCALE."../modules/toolXlate/locale/en_US/language.php");
       if (file_exists(BIZUNO_LOCALE."../modules/$modID/locale/en_US/language.php")) {
           require    (BIZUNO_LOCALE."../modules/$modID/locale/en_US/language.php"); // populates $lang
           $output = $lang;
       }
       if ($myLang == 'en_US') { return $output; }
       // @todo this doesn't work, need to extract the module and go from there
       if (file_exists(BIZUNO_LOCALE."$myLang/ext/$modID/language.php")) {
           require    (BIZUNO_LOCALE."$myLang/ext/$modID/language.php"); // populates $lang
